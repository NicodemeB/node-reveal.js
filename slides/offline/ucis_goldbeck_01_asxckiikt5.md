...
- In the introduction of usability we saw mostly two things:
 - How to measure usability, which measurement to use ( Speed,Efficiency, Learnability, Memorability,User preference)
  - first one:  testing the validity of RTA (one usability tool) using an other tool
...
visual system, psychology, in psycholinguistics, marketing, as an input device for human-computer interaction, product design, rehabilitative application

Javal: - Reading series of short stops and quick saccades

some evolution follows: less intrusive eye tracking, way to look depens on task

1980's : 
 - How users search for commands in computer menus
 - Use eye-tracking results in real time to help disabled users <- Which is what we are going to compare in the seconde study
...
- Fine:
  - 53% valid accounts
  - 28% of approximate matches (identified coarce but not fine)
  - 15% misstatement (haven't identified proper coarce)  
  - 4% fabrication
...
- interesting trend: subjects tended to produce more valid accounts and commit fewer fabrications in the complex tasks than in the simple tasks. 
...
- Validty doesn't vary with complexity
- More information are provided for a complex task 
...
Notes: 
there are two versions of gazetalk
...
Talk about what it means for music 
 ...
 -ixi software is a interdisciplinary research project screen-based interfaces as digital musical instruments
...
- depends on the agent -> a violin offer more action to human than dog

- Norman introduces the idea of perceived affordances (narrower than previous) & more interresting to us
...
Motor learning scholars note that
effective chunking is an important characteristic of high-level
motor skill performance (autonomous stage), such as driving a
...
## In Tenis: used to integrate the entire motor pattern of groundstrokes

- groundstroke elements into distinct sub-units:
 - preparation
 - forward swing
 - follow-through/recovery
- focus on fewer welllearned sub-units 
...
Notes: 
one referance - distance
under all conditions

he adjust based on that reference without having to think about everything again
chuck: hit that distance

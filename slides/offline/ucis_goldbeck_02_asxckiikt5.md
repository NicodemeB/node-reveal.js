...
Notes: 
- Intro to design, 
- Then with this introduction; methodologies around design 
- Finally, a case around design issues
...
- Should be transferred for every population; are you working with bankers, includes bankers, 
...
Notes:
- The idea came from the 1970s (Scandinavian countries, From the Social Democratic Movement), when factory workers asked a computer scientists how they were going to develop a computer system that is used by the factory workers
...
- Make profiles of this population
- Used in military
...
Notes:
Depends on where we are in the design process:
...
What if you're just some guy building an application in your basement with limited resources? What's the best way to do design methods ?

- **Think** about to whom the application is addressed, **talk** to these people,
    - ASAP
...
ASAP : talk to **users** ASAP else they could be **hesitant** to give their ideas if the **prototype** is **too finished** = you spend lots of time, 
...
What's the hardest part of the design ?
...
- Used methodology
- Example : Building **authentication system** for mobile devices
    - could take lot of time
    - "useless" if the platform already supports different ways to do so
    - **Is it worth ?**
...
Notes:
- The design of the computer on wheels for physicians (hey why use laptops ¯\\\_(ツ)_/¯), is an example where **user-centered design was not followed**
...
- Cannot know enough the users to solve all issue on the design phase
...
- Interviews are not precise enough, more questions when designing and do not have the answer, have **to guess**
...
Notes:
* Include the customer is ok but at some point, designers have to do their job and come up with new ideas
...
- Goal : observe if **lambda users** can **understand** the warning and **react accordingly**
...
* Nowadays, some applications are luring users away from self signed certificate saying that it automatically mean that there is a MitM attack and making it very hard, when possible, to check the fingerprint. This is a problem for us in Cybersecurity. Self-signed does not always mean less secure.
	...
	* you do not need to trust a central authority for your PKI. So one could assume that when following the right procedure (that is a costly one of checking the fingerprint through an authenticated channel that ensure integrity), it is more secure
	* in some browsers, you cannot even accept the exception. You need to accept it in private navigation and then continue your browsing in the main window.
I have also seen android applications that either do not support at all self signed (rocket chat, matter most) or only allow you to check a box "do not check certificate" which removes all the benefits of TLS
...
Notes:
- A user is asked to react to an alert/error regarding SSL on Firefox 2/3 browser
...
- Idea : browsers should prevent users to continue, but : 
...
- Test different User Interface
- Compare the results 
- User interfaces play a role in security 

Notes:
Paper idea is to test different designs of the browser to see if there is a behavior change for the users
...
- Wanted to change that,
    - Make it different ; change the design to **read**
...
VVVVV
<!-- .slide: data-background="  #456789" -->
###  The Jigsaw puzzle experiment analysis cont'
- Hazy Ideas
	- Thinking of different designing scenarios
	- These scenarios are in the mind of the designer
...
- Key points to retain about this experiment:
    ...
    - Design by iteration may help to improve the initial ideas
...
- Scenarios
	...
	- Business unit involved being unaccustomed to providing requirements for technology
- A possible outcome from the development team
	- Go off for months & create something that users end up of rejecting
...
Notes: 
    - Brain storming: students trying to fetch ideas from imaginary  
...
Notes:
All available in the presenter's notes 

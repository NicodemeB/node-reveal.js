...
Interesting effect we __might not think about__ when __notifying__ the user 
...
__use case for journalists__: __encryption__and __authentication as access control__
...
- Regardless the permission

Notes: 
- Alice install a program and they can access the home directory 
- Then she installs a server program 
	- She gave access to Bob with an URL 
	- She has just granted Bob the authority to read her files
...
- Accomplish tasks 
	- They will always do it in the easiest way 
...
- Disclosing to bud your exact birthday = 
	- Based on the IP address, they could know who you are 
	- IP + birthday = can almost profile the exact person you are 
- They could just ask are you 21 or not 
	- Easy to lie in both cases 
...
- Granting 
	- User has to explicitly grant the access 
- Understanding 
	- User need to clearly understand that he granted an access 
	- Can't be a "hidden" action granting it : must be explicit 
...
__chrome cleanup tool prompt__: displayed on windows system when it detects __malware/addware__ that change __the internet setting or chrome browser__ settings and ask to remove malware and reset settings
security message triggered by system events without regard to what’s the user is doing
so =>
/!\this is used /!\ to check for __permission__ from a __malware that has authority__(show def above) but maybe not __permission for that task__

- remember the example of the video:
 - authority to dial the phone but not to call in France 
 -  permission was to dial a friend of her
 - not giving authority by dialing yourself was good security
    
making the easy way the safest way is not trivial
human are not secure and miss information
look at neuroscience 

pretty cool website and ideas but nothing new since 2017
...
- seems the simplest & secure way but isn't
...
chrome cleanup tool prompt checking for permission from malware/spyware that has authority

security message triggered by system events without regard to what’s the user is doing

that's why it's useful to look at neurology
...
Notes: 
- wait until he shows conclusions and habituation at 9:20
...
- with each display of a warning our brain pay less and less attention until we stop seeing it = warning fatigue
...
- displaying information isn't making the user look at it
- the user could very well make abstraction of a part of the screen
- works with access control but not only

Notes:
we will see in the 4 studies that even if the access control rule are clearly accessible, it doesn't mean the user will look at it
...
- Other = the user, Other users and systems
...
- How can the interface provides timely access to information about these authorities ? 
...
- IE: PayPal still has authority on the money even if we "received" the money 
...
- IE : malware, phishing, ... : share private info but not the correct website or soft 
...
- If they know with what they are interacting 
	...
	- Otherwise : NO PERSO INFO (like passwords)
...
- Problem with security :
	- They are ways to control permission and set policies
...
- Policies 
	- Access policies : setting permission for the allowed people to access a file / document
- Express
- Expression 
...
- level of detail 
	...
	- Think the level of detail, our user needs
- Affected and unaffected 
	- IE: applications on the mac are in reality folders 
		- If he changes something in preferences, the files in this folder could change but does he need to see it ? or they don't care ? 
...
- The point here is that the user should know what he is looking at and what they are interacting with
...
- Distinguish 
	- IE: Distinguish phishing from real email from her bank 
		- Objects : phishing and non-phishing emails
		- Same for the actions (desirable and non-desirable actions)
		- *The interface should reveal that !*
- Means of identification 
	- IE: the phishing website may use a URL similar but a bit different from the real one 
...
- Prevent 
	- Font can help to avoid confusing capital i and a lower case l 

VVVVV

## Conclusion 

- Make it easier for users to control access to their resources 
...
paper by people who tried to study exactly what interests us
...
- Main Hypothesis: 
 - Users who see information about access-control permission settings on the __main interface notice permission errors more often than__ users who have to proactively open a __second interface__ to view permission

Notes:
in conclusion of the video of the course it is recommended to: "Make it easier for users to control access to their resources"
...
- "__When permission is changed__ to an error state by a __3rd party__, users who see permission information under the photos/albums or on the sidebar notice errors more often than users who see permission information only if they click to a second page."
...
- "The type of error, too much permission or too few, has an effect on the number of errors noticed. "
- "Participants who see permission information under the photos/albums or on the sidebar can __recall__ those permission better than participants who see permission information only if they click to a second page. "
...
- We also built a new permission modification interface that shows the permission for every album on a single page.

- permission cannot be expressed for individual users or photographs.
...
intended to __distract__the participants
...
Notes:
Asked to recall permission they worked with __For each combination of albums, group, and permission__ the participant could answer __True, False, or Not Sure__

__Eye tracker__ to see if they check the permission error on main pannel1, 2 and 3
...
- Subjects guessed it was a spotting errors game
- Only 67% noticed permission errors without prompt
- For comparison 86% of errors in the titles were corrected
- Didn't use display

Notes:

__Because of training__ subjects understood spotting errors that mattered __but not authorization errors__
...
_"Participants who see permission information on the main screen are, __in the absence of an error__, less likely to open the permission modification screen than users who have to proactively open a second interface to view permission."_
...
- 53% of participants corrected permission on 3 or less of the 12 tasks*
...
Notes: 
because no participant engaged in check listing type behavior

*before being prompted 

we can see they did even worse than before
...
Need to be on an equal footing if we want to compare them
wording of tasks caused participants to check permission on some tasks more than others, suggesting that participants did not have consistent priming.
...
- 11/31 checked permission on more than 50% tasks

Notes:

suggesting that for the majority of participants permission __remained a secondary task__
...
- In real life doing anything has a cost
 - You might not check carefully every setting
...
- observed that some participants internalized the need to check permission while others did not finally not suitable because of same reason as before ->
...
Some participant didn't think it was their responsibility to change the settings (role play misunderstood)

The __per-album policy__ gave participants __less priming towards fixing permission__ but __not consistent__ 
...
Notes:
what's the best way for journalists to ensure the respect of the confidentiality of the information that is being shared between themselves and their sources in a usable way
...
- choice of communication technology is the driver by the source 
...
Notes:
afraid to scare the source, they just follow them

wait a lot before getting infos from long known sources => change in nature of the relationship

journalists don't realize risks can come from a third party
